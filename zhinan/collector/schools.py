...
class EnrollmentCollector(BaseCollector):
    """各校总人数采集器 — 从内建数据更新 total_enrollment。

    数据来源：各校官网"学校简介"页面的公开数据。
    """
    ...
    async def collect(self) -> int:
        ...
        logger.info("Starting enrollment update (%d schools with known data)", len(ENROLLMENT_DATA))
        count = 0
        updated = 0

        for school_name, enrollment in ENROLLMENT_DATA.items():
            row = await self.db.fetch_one(
                "SELECT id, total_enrollment FROM schools WHERE name = ?",
                (school_name,),
            )
            if row:
                count += 1
                if row["total_enrollment"] != enrollment:
                    await self.db.execute(
        ...
        logger.info("Enrollment update done: %d matched, %d updated", count, updated)
        return updated
